...
main(int argc, char ** argv)
{
	...
	if(!(f_tmp=fopen(argv[2], "r")))
	{
		printf("Nie moge otworzyc pliku: %s\n", argv[2]);
		return -3;
	}else{
		fclose(f_tmp); 
		wym_B= read_wymiary(argv[2]);	
	}
	...
	for(i=0; i< ile_watkow; i++)
	{
		...
		strncpy(dane_watkow[i].f2,argv[2],MAX_NAME);	
	...
	*/
	...
	fprintf(pom,"\n%2.5f\t",duration);
	fprintf(pom,"%2.5f\t",count_duration);
	fprintf(pom,"%2.5f\t",duration - count_duration);
